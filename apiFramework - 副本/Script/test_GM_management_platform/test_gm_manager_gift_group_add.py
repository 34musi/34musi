...
"""
@author:    yuless
@file:      test_gm_manager_gift_group_add.py
@time:      2022/3/25 16:00
...
class TestClassuser():
    ...
    def setup_class(self):
        before = time.ctime()
        logger.info(" test_gm_manager_gift_group_add before time is : %s " % before)
        self.objyaml = OperationYaml()
        self.interface_info = \
        self.objyaml.aloneReadYaml('Data', 'test_gm_management_platform.yaml', 'test_gm_manager_gift_group_add')[0]
    ...
    def test_gm_manager_gift_group_add(self):
    ...
    def teardown_class(self):
        after = time.ctime()
        logger.debug(" test_gm_manager_gift_group_add after time is : %s " % after)


if __name__ == '__main__':
    pytest.main(['-s', '-v', ' test_gm_manager_gift_group_add.py'])
