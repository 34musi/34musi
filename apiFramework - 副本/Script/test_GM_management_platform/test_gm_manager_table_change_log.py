...
class TestClassuser():
    ...
    def setup_class(self):
        before = time.ctime()
        logger.info("test_gm_manager_table_change_log before time is : %s " % before)
        self.objyaml = OperationYaml()
        self.interface_info = self.objyaml.aloneReadYaml('Data', 'test_gm_management_platform.yaml', 'test_gm_manager_table_change_log')[0]
    ...
    def test_gm_manager_table_change_log(self):
    ...
    def teardown_class(self):
        after = time.ctime()
        logger.debug("test_gm_manager_table_change_log after time is : %s " % after)


if __name__ == '__main__':
    pytest.main(['-s', '-v', 'test_gm_manager_table_change_log.py'])
